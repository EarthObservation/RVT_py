...
def test_slope_aspect(
    input_dem_path, output_path, vertical_exaggeration_factor=1, output_units="degree"
):
...
def test_multi_hillshade(
    input_dem_path, output_path, nr_directions=16, sun_elevation=35
):
    input_dem_dataset = rio.open(input_dem_path)
    t = input_dem_dataset.transform
    x_res = t[0]
    ...
    multi_hillshade_arr = rvt.vis.multi_hillshade(
        ...
        resolution_x=x_res,
        resolution_y=y_res,
        number_of_directions=nr_directions,
        sun_elevation=sun_elevation,
    )
...
def test_slrm(input_dem_path, output_path):
    ...
    slrm_arr = rvt.vis.simple_local_relief_model(input_dem_arr)
...
def test_sky_view_factor(
    input_dem_path, output_svf_path=None, output_asvf_path=None, output_opns_path=None
):
    input_dem_dataset = rio.open(input_dem_path)
    t = input_dem_dataset.transform
    x_res = t[0]
...
def test_sky_illumination(input_dem_path, output_path):
    ...
    y_res = -t[4]
    input_dem_arr = input_dem_dataset.read()[0]
    skyillumination_arr = rvt.vis.sky_illumination(
        dem=input_dem_arr, resolution=x_res, compute_shadow=True
    )
