...
svf_dict = rvt.vis.horizon_visualizations(
    dem=input_dem_arr, resolution=x_res, compute_svf=True, compute_opns=True
)
svf_arr = svf_dict["svf"]
layers_manual.create_layer(
    ...
    opacity=25,
    image=svf_arr,
)  # you could also input image_path
...
slope_dict = rvt.vis.slope_aspect(
    dem=input_dem_arr,
    resolution_x=x_res,
    resolution_y=y_res,
    output_units="degree",
...
hillshade_arr = rvt.vis.hillshade(
    dem=input_dem_arr, resolution_x=x_res, resolution_y=y_res
)
layers_manual.create_layer(
...
layers_auto.read_from_json_file(
    file_path=blender_file
)  # build BlenderCombination from file
# when building_blender from file single BlenderLayer image and image_path are None
layers_auto.add_dem_path(
    input_dem_path
)  # needed when save_visualizations is True and save_rander_path is not None
...
layers_auto.add_dem_arr(
    dem_arr=input_dem_arr, dem_resolution=x_res
)  # needed when save_visualizations is False
