...
def test_analytical_hillshading(input_DEM_path, output_path):
    ...
    hillshading_arr = RVT_vis_fn.analytical_hillshading(dem=input_DEM_arr, resolution_x=x_res,
...
def test_multiple_directions_hillshading(input_DEM_path, output_path):
    ...
    y_res = -t[4]
    input_DEM_arr = input_DEM_dataset.read()[0]
    multi_hillshading_arr = RVT_vis_fn.multiple_directions_hillshading(dem=input_DEM_arr, resolution_x=x_res,
...
def test_slrm(input_DEM_path, output_path):
    ...
    slrm_arr = RVT_vis_fn.SLRM(input_DEM_arr)
    profile = input_DEM_dataset.profile
...
def test_sky_illumination(input_DEM_path, output_path):
    ...
    skyillumination_arr = RVT_vis_fn.sky_illumination(dem=input_DEM_arr, resolution=x_res)
    end_time = time.time()
