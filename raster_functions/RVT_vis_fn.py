...
def analytical_hillshading(input_DEM_arr, resolution, sun_azimuth=315, sun_elevation=35, ve_factor=1):
    if sun_azimuth > 360 or sun_elevation > 90 or sun_azimuth < 0 or sun_elevation < 0:
        raise RVT_error("analytical_hillshading: sun_azimuth must be [0-360] and sun_elevation [0-90]")
    ...
    slope, aspect = slope_aspect(input_DEM_arr, resolution, ve_factor)
...
def slope_aspect(input_DEM_arr, resolution, ve_factor, output_units="radian"):
    if ve_factor <= 0:
        raise RVT_error("slope: Vertical exagerration must be positive number!")
    ...
    if output_units == "percent":
        ...
        raise RVT_error("calculate_slope: Wrong function input 'output_units'!")
    ...
    # 270    90
    #    180
    dzdy[dzdy == 0] = 10e-9  # important for numeric stability - where dzdy zero is, make tangens to really high value
